...
def draw_disease_boxes(original_pil: Image.Image, heatmap, disease_name):
    ...
    for cnt in contours:
        ...
        if area > 300:  # remove tiny noise
            cv2.rectangle(annotated, (x, y), (x + box_w, y + box_h), (255, 0, 0), 4)  # thick red box
            # Optional label on box
            cv2.putText(annotated, disease_name[:10], (x, y-8), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
            spot_count += 1
...
with col1:
    input_method = st.radio("Choose input method:", ["📤 Upload Image", "📷 Live Camera"], horizontal=True)
...
if image is not None:
    ...
    if st.button("🔍 Predict Disease", type="primary", use_container_width=True):
        with st.spinner("Analyzing + drawing disease spots..."):
            ...
            annotated_img, spot_count = draw_disease_boxes(image, heatmap, predicted_class.replace("_", " ").title())
            ...
            with col_a:
                st.image(image.resize((380, 380)), caption="Original Leaf", use_container_width=True)
            with col_b:
                st.image(annotated_img.resize((380, 380)), caption="🔴 Disease Spots Detected", use_container_width=True)
